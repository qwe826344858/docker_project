...
# 构造函数
def __init__(pool_connect,host= '',port= 0):
    if host != '' or port != 0:
        redis_host = host
        redis_port = port
...
def getRedisHash(name, key):
    try:
        ...
        print(f" 获取Hash value失败! name:{name} key:{key} errMsg:{e}")
        return False, {}
...
def getRedisAllHash(name):
    try:
        ...
        print(f" 获取所有Hash value失败! name:{name} errMsg:{e}")
        return False, []
...
def delRedisHash(name, key):
    try:
        ...
        print(f" 删除Hash value失败! name:{name} key:{key} errMsg:{e}")
        return False
...
def setExpireTime(name, expire_time):
    ...
    print(f" name:{name} expire_time:{expire_time} ret:{ret}")
    return True
